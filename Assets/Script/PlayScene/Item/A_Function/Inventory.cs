...
public class Inventory : MonoBehaviour
{
    ...
    */
    ...
    public void GetItem(Item _item)
    {
        for (int i = 0; i < items.Length; i++)
        {
            if(items[i] == null)
            {
                ...
                items[slotNum] = _item;
                questManager.AddCollectQuest(_item.itemNumber);
    ...
    public void AddItems(Item _items)
    {
        ...
        CheckUseableSlot?.Invoke(_items);
        ...
        if (UseableSlot)
        {
            for (int i = 0; i < items.Length; i++)
            {
                ...
                //같은 아이템이 있는 슬롯 찾기
                else if (items[i].itemName == _items.itemName)
                {
                    //새로 습득한 아이템이 현재 슬롯에 중첩 가능한지 체크
                    GoodsOverlap?.Invoke(i, _items);

                    //가능하면 아이템 중첩
                    if (isOverlap)
                    {
                        questManager.AddCollectQuest(_items.itemNumber);
        ...
        else
        {
            ...
            for (int i = 0; i < items.Length; i++)
            {
                if (items[i] == null)
                {
                    //items[i] = _potion;
                    slotNum = i;
                    invenItems[slotNum].slotItem = _items.itemNumber;
                    items[slotNum] = _items;
                    questManager.AddCollectQuest(_items.itemNumber);
                    AddUpdateUI?.Invoke(slotNum);
